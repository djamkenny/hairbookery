...
export const Services = () => {
  ...
  return <section id="services" className="py-16 md:py-24 relative overflow-hidden">
      {/* Background Effects */}
      ...
      <div className="container mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-12 md:mb-16">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass-card mb-4 animate-fade-in">
            <span className="text-xs md:text-sm font-medium text-primary">Simplifying Appointments for Service Providers</span>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 mb-12 md:mb-16">
          {filteredServices.map((service, index) => <div key={service.id} className="animate-fade-in group" style={{
          animationDelay: `${index * 100}ms`
        }}>
              <ServiceCard title={service.title} description={service.description} price={service.price} duration={service.duration} image={service.image} features={service.features} className="gradient-card h-full hover:scale-[1.02] transition-all duration-300" />
            </div>)}
        </div>
        
        <div className="text-center">
          <Link to="/specialists" className="group inline-block">
            <Button size="lg" className="animate-fade-in relative overflow-hidden bg-gradient-to-r from-primary to-accent-purple hover:shadow-glow transition-all duration-300 group-hover:scale-105 px-8 py-6 text-base">
