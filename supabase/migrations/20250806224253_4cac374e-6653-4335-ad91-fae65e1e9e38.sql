...
('Men\'s Grooming', 'Haircuts, beard styling, and men\'s treatments', 7, 'user'),
...
UPDATE services SET category = CASE
  ...
  WHEN LOWER(name) LIKE '%men%' OR LOWER(name) LIKE '%beard%' OR LOWER(name) LIKE '%fade%' THEN 'Men\'s Grooming'
